...
setup_arbiter_options() {
    ...
    MAIN_CLASS="cn.pandadb.server.PandaServerStarter"
...
assemble_command_line() {
  retval=("${JAVA_CMD}" "-cp" "${CLASSPATH}" "${JAVA_OPTS[@]}" "-Dfile.encoding=UTF-8" "${MAIN_CLASS}" \
          "$NEO4J_HOME" "$NEO4J_CONF/neo4j.conf")
}
...
main() {
  setup_environment
  NEO4J_HOME=$(dirname $(pwd))
  NEO4J_CONF=$NEO4J_HOME/conf
  NEO4J_LIB=$NEO4J_HOME/lib
  NEO4J_LOGS=$NEO4J_HOME/logs
  NEO4J_RUN=$NEO4J_HOME/run
  NEO4J_DATA=$NEO4J_HOME/data
  NEO4J_PLUGINS=$NEO4J_HOME/plugins
  NEO4J_CERTS=$NEO4J_HOME/certificates
  readonly NEO4J_HOME NEO4J_CONF NEO4J_LIB NEO4J_LOGS NEO4J_RUN NEO4J_DATA NEO4J_PLUGINS NEO4J_CERTS
